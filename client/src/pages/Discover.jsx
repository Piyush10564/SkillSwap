...
export default function Discover() {
  ...
  return (
    <div>
      <div>
        <h2 className="app-section-title text-xl font-semibold text-strong">Discover skills & mentors</h2>
        <p className="text-sm text-muted">Search people who can teach you what you want, and who want to learn from you.</p>
      </div>
      ...
      <div className="mt-6 page-surface p-4">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <input
            ...
            className="rounded-xl border soft-border bg-transparent px-4 py-2 text-sm text-strong placeholder:text-soft focus:border-[color:var(--accent)] focus:bg-transparent focus:outline-none focus:ring-2 focus:ring-[rgba(255,107,74,0.18)] focus-ring"
          />
          <select
            value={filters.category}
            onChange={(e) => setFilters({ ...filters, category: e.target.value })}
            className="rounded-xl border soft-border bg-transparent px-4 py-2 text-sm text-strong focus:border-[color:var(--accent)] focus:bg-transparent focus:outline-none focus:ring-2 focus:ring-[rgba(255,107,74,0.18)] focus-ring"
          >
          ...
          <button
            ...
            disabled={loading}
            className="rounded-full surface-accent px-4 py-2 text-sm font-medium text-white shadow-sm hover:brightness-105 disabled:opacity-50"
          >
      ...
      <div className="mt-6">
          {loading ? (
          <div className="text-center py-12 text-soft">Searching...</div>
          ) : skills.length === 0 ? (
          <div className="card-surface p-12 border-dashed soft-border text-center">
            <p className="text-sm text-muted">No skills found</p>
            <p className="text-xs text-soft mt-1">Try adjusting your search filters</p>
          </div>
        ) : (
              ...
              <div key={skill._id} className="card-surface p-4">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-start gap-3">
                    <div className="h-12 w-12 rounded-full brand-gradient flex items-center justify-center text-sm font-semibold text-white">
                      {skill.owner?.name?.substring(0, 2).toUpperCase() || 'U'}
                    </div>
                    <div className="flex-1">
                      <h3 className="font-semibold text-strong">{skill.name}</h3>
                      <div className="flex items-center gap-2">
                        <p className="text-sm text-muted">{skill.owner?.name}</p>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-full surface-accent px-4 py-2 text-sm font-medium text-white shadow-sm hover:brightness-105 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-full border soft-border bg-white/70 px-3 py-2 text-sm text-strong hover:bg-white focus-ring"
                    >
                      Request to learn
                    </button>
                  </div>
